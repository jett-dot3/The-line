...
# HELPERS
# ─────────────────────────────────────────
def safe_get(url, headers=None, params=None, timeout=15, label=""):
    try:
...
def fetch_nba_stats():
    ...
    r = safe_get(def_url, headers=headers, params=def_params, label="NBA.com team defense")
    if r:
        try:
            data = r.json()
    ...
    r = safe_get(player_url, headers=headers, params=player_params, label="NBA.com player stats")
    if r:
        try:
            data = r.json()
    ...
    r = safe_get(track_url, headers=headers, params=track_params, label="NBA.com tracking drives")
    if r:
        try:
            data = r.json()
            hdrs = data["resultSets"][0]["headers"]
    ...
    r = safe_get(matchup_url, headers=headers, params=matchup_params, label="NBA.com defensive matchups")
    if r:
        try:
            data = r.json()
            hdrs = data["resultSets"][0]["headers"]
...
def fetch_covers():
    ...
    # NBA team ATS records — accessible page
    nba_ats_url = "https://www.covers.com/sport/basketball/nba/teams/main"
    r = safe_get(nba_ats_url, headers=headers, label="Covers NBA ATS")
    if r:
        txt = r.text
        # Look for ATS record patterns
        start = txt.find("ATS")
        results["nba_ats"] = truncate(txt[max(0, start-200):start+3000]) if start > -1 else ""

    # MLB team ATS records
    mlb_ats_url = "https://www.covers.com/sport/baseball/mlb/teams/main"
    r = safe_get(mlb_ats_url, headers=headers, label="Covers MLB ATS")
    if r:
        txt = r.text
        start = txt.find("ATS")
        results["mlb_ats"] = truncate(txt[max(0, start-200):start+3000]) if start > -1 else ""

    # Covers public consensus — NBA
